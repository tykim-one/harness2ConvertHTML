...
- **Task 6**: save the extracted output as a repo-managed intermediate artifact for the sample
  Description: save the extracted output as a repo-managed intermediate artifact for the sample
  Status: pending
  Role: team-executor
- **Task 7**: define the first IR/schema for this sample
  Description: define the first IR/schema for this sample
  Status: pending
  Role: team-executor
- **Task 8**: transform pyhwp XML/model output into that IR
  Description: transform pyhwp XML/model output into that IR
  Status: pending
  Role: team-executor
- **Task 11**: preserve runtime contract verification and run artifact reporting
  Description: preserve runtime contract verification and run artifact reporting
  Status: pending
  Role: team-executor
- **Task 15**: preserve editability and binding structure
  Description: preserve editability and binding structure
  Status: pending
  Role: team-executor
- **Task 19**: a first IR exists
  Description: a first IR exists
  Status: pending
  Role: team-executor
- **Task 21**: a fresh run artifact is created and verifiable through the harness.
  Description: a fresh run artifact is created and verifiable through the harness.
  Status: pending
  Role: team-executor
...
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):
   ...
   `omx team api send-message --input "{"team_name":"implement-the-first-true-hwp-d","from_worker":"worker-3","to_worker":"leader-fixed","body":"ACK: worker-3 initialized"}" --json`
...
12. Write `{"state": "idle", "updated_at": "<current ISO timestamp>"}` to `/Users/tykim/Documents/workspace/onelineai/convertToHtmlHarness/.omx/state/team/implement-the-first-true-hwp-d/workers/worker-3/status.json`
...
1. List mailbox:
   `omx team api mailbox-list --input "{"team_name":"implement-the-first-true-hwp-d","worker":"worker-3"}" --json`
2. For each undelivered message, mark delivery:
   `omx team api mailbox-mark-delivered --input "{"team_name":"implement-the-first-true-hwp-d","worker":"worker-3","message_id":"<MESSAGE_ID>"}" --json`
...
Example: omx team api send-message --input "{"team_name":"implement-the-first-true-hwp-d","from_worker":"worker-3","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
