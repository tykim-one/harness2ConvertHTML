...
- **Task 1**: Implement the first true HWP-direct-to-HTML pipeline for the sample 260121 일일 주요
  Description: Implement the first true HWP-direct-to-HTML pipeline for the sample 260121 일일 주요경제지표.hwp. Final goal: the original HWP must be directly parsed
  Status: pending
  Role: team-executor
- **Task 2**: the pipeline must generate the final editable HTML from extracted structure
  Description: the pipeline must generate the final editable HTML from extracted structure
  Status: pending
  Role: team-executor
- **Task 3**: the primary generation path must be HWP to IR to HTML, not reference-copy reuse.
  Description: the primary generation path must be HWP to IR to HTML, not reference-copy reuse. Current starting point: pyhwp/hwp5proc is installed and hwp5proc xml can parse the sample HWP
  Status: pending
  Role: team-executor
- **Task 5**: the harness must remain the evaluation loop. Required outcomes: add a stable ext
  Description: the harness must remain the evaluation loop. Required outcomes: add a stable extraction wrapper that runs pyhwp/hwp5proc from the pipeline
  Status: pending
  Role: team-executor
- **Task 9**: implement the first renderer step that uses IR to generate editable HTML artifac
  Description: implement the first renderer step that uses IR to generate editable HTML artifacts
  Status: pending
  Role: team-executor
- **Task 17**: if full HTML generation cannot be completed in one pass, the minimum acceptable 
  Description: if full HTML generation cannot be completed in one pass, the minimum acceptable midpoint is repo-managed extraction artifact plus IR plus renderer entrypoint stub wired into the pipeline. Definition of done: the pipeline directly parses the sample HWP
  Status: pending
  Role: team-executor
- **Task 18**: a repo-managed extraction artifact exists
  Description: a repo-managed extraction artifact exists
  Status: pending
  Role: team-executor
- **Task 20**: generated HTML is produced from IR, not from reference-copy reuse
  Description: generated HTML is produced from IR, not from reference-copy reuse
  Status: pending
  Role: team-executor
...
2. Send startup ACK to the lead mailbox BEFORE any task work (run this exact command):
   ...
   `omx team api send-message --input "{"team_name":"implement-the-first-true-hwp-d","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK: worker-1 initialized"}" --json`
...
12. Write `{"state": "idle", "updated_at": "<current ISO timestamp>"}` to `/Users/tykim/Documents/workspace/onelineai/convertToHtmlHarness/.omx/state/team/implement-the-first-true-hwp-d/workers/worker-1/status.json`
...
1. List mailbox:
   `omx team api mailbox-list --input "{"team_name":"implement-the-first-true-hwp-d","worker":"worker-1"}" --json`
2. For each undelivered message, mark delivery:
   `omx team api mailbox-mark-delivered --input "{"team_name":"implement-the-first-true-hwp-d","worker":"worker-1","message_id":"<MESSAGE_ID>"}" --json`
...
Example: omx team api send-message --input "{"team_name":"implement-the-first-true-hwp-d","from_worker":"worker-1","to_worker":"leader-fixed","body":"ACK: initialized"}" --json
