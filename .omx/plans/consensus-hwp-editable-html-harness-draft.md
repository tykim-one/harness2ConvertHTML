...
- Capture-pack gating:
  - if capture assets are present, the run is eligible for full fidelity review
  - if capture assets are absent, `scripts/run_sample.py` may still produce a `conversion-only / review-blocked` run, but that run cannot satisfy final acceptance criteria tied to fidelity review

The exported-state contract to preserve is:
...
- `data-semantic-field`, `data-table-field`, `data-table-id`, and `data-cell-key` addressing remain intact for all mapped blocks/cells declared by `today_major_economy/extracted_fields.json`.
- The generated bundle resolves fonts from copied `assets/fonts/`.
- Exported state preserves the concrete runtime structure: `meta`, `blocks`, `semanticFields`, and `tables[tableId].cells[cellKey]`.
...
3. The workflow preserves `data-cell-key`-based table lookup and export behavior alongside `data-table-id`.
4. The generated output preserves editable runtime behavior: edit mode toggle, field application path, runtime font injection path, and JSON export via `collectState()`.
5. Bundled fonts resolve from the copied artifact bundle with no dependency on external office/image binaries.
6. `today_major_economy/` remains byte-for-byte unchanged throughout the run.
7. The review bundle is sufficient for manual fidelity review against the capture pack and frozen reference.
8. Runs without a capture pack are explicitly marked `conversion-only / review-blocked` and cannot be treated as final fidelity acceptance runs.
...
3. Verify the generated HTML contains required hooks and selectors:
   ...
   - `collectState`
   - `runtime-font-faces`
4. Verify all semantic and table addresses declared in `today_major_economy/extracted_fields.json` resolve in generated output using this derivation rule:
   - if an entry has `semantic_field`, verify a matching `data-semantic-field`
   - if an entry has `table_field` matching `tables.<tableId>.r<row>c<col>`, derive `cell_key` as `r<row>c<col>` from the suffix and verify a matching `data-table-id="<tableId>"` plus `data-cell-key="<cell_key>"`
   - if an entry only has `block_id`, verify the runtime preserves the corresponding block binding and exported-state entry
5. Verify exported JSON from the runtime preserves the concrete structure `meta`, `blocks`, `semanticFields`, and `tables[tableId].cells[cellKey]`.
...
8. Verify runs without capture assets are marked `conversion-only / review-blocked`, and that final fidelity review is blocked until capture assets exist.
